...
class Subsystem extends React.Component {
  ...
  render() {
    const item = <Item.Group>
      <Item>
        <Item.Image className='subsystem__image' size='tiny' src={this.getImage(this.props.name)} />
        <Item.Content>
          <Item.Header as='a'>{this.getHeader(this.props.name)}</Item.Header>
          <Item.Description>
            {this.getDescription(this.props.name)}
          </Item.Description>
        </Item.Content>
      </Item>
    </Item.Group>
    return this.getSegment(this.props.name, item)
  }
}
